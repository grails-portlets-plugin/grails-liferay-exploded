...
liferayHome = ant.project.properties."environment.LIFERAY_HOME" 
...
def createContextXml(String filename, String pathAttr, String docBaseAttr, boolean reloadableAttr)
{
    def xmlWriter = new StreamingMarkupBuilder()
...
target(main: "Deploys exploded war's context file to LIFERAY_DEPLOY") {
   ...
   depends(parseArguments, war)
	...
	String filename = "${liferayHome}/deploy/${grailsAppName}.xml"

	//ant.echo("Creating filename: $filename")

	// create context.xml file for our fast deployment
    createContextXml(filename, grailsAppName, explodedDir, true)
}
